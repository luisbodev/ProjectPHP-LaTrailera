        ...
        <div class="bg md:flex md:justify-between md:px-2 md:py-1 md:items-center w-screen">
            <div class="flex items-center justify-between px-2 py-1 md:px-0 md:py-0">
                <div class="flex items-center">
                    <a href="controlClienteVentana.php"><img class="h-12" src="img/logo/Logo-LaTrailera2.svg" alt="Logo La Trailera"></a>
                </div>
                <a href="controlClienteVentana.php"><h1 class="text-white text-2xl font-bold">LA TRAILERA</h1></a>
                <div class="md:hidden">
                ...
                <div id="opciones" class="hidden px-1 pb-1 md:flex md:p-0">
                    <?php
                        if($currentPage == 'Inicio'){?> <a href='#' class='block px-2 font-semibold rounded text-gray-800 bg-gray-200'>Inicio</a> <?php } else { ?> <a href='controlClienteVentana.php' class='block px-2 text-white font-semibold rounded hover:text-gray-800 hover:bg-gray-200'>Inicio</a> <?php }
                        if($currentPage == 'Envios'){?> <a href='#' class='block mt-1 px-2 font-semibold rounded text-gray-800 bg-gray-200 md:mt-0 md:ml-2'>Envios</a> <?php } else { ?> <a href='controlEnvioCliente.php' class='block mt-1 px-2 text-white font-semibold rounded hover:text-gray-800 hover:bg-gray-200 md:mt-0 md:ml-2'>Envios</a> <?php }
                    ?>
                    <a href="controlClienteVentana.php?c=1" class="block mt-1 px-2 text-white font-semibold rounded hover:text-gray-800 hover:bg-gray-200 md:mt-0 md:ml-2">Cerrar Sesión</a>
                    <div class="block mt-1 px-2 text-white bg-orange-500 font-semibold rounded md:mt-0 md:ml-2">Usuario: <span id="userName" class="underline"><?php echo $_SESSION["cliente"]; ?></span></div>
                </div>
            </div>
        </div>
        <script>
